...
try {
 ...
 image.getBuffer(Jimp.MIME_JPEG, (err, buf) => {
  ...
  form.append('image', buffer, { filename: 'removebg.jpg' });
    const { data } = await axios.post(`${global.api.tool}ai/removebg`, form, {
      headers: {
        ...form.getHeaders(),
...
try {
 if (event.type != "message_reply" || !event.messageReply.attachments[0].url) return;
  message(await font(language.request.replace("{0}", "remove background")), "✅");
  const leiamZ = `${cache}rmbg_${event.senderID}.png`;
