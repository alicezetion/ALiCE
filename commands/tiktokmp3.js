...
try {
  ...
  const leiamI = `${cache}tiktokmp3_${event.senderID}.mp3`;
...
fs.writeFileSync(leiamI, Buffer.from(leiamS, 'utf-8'));
 ...
 log.error(`[ ${this.alice.command} ] » ${error}`);
  return bot.chat(`[ ${this.alice.command} ] » ${language.error}`, event.threadID, () => fs.unlinkSync(`${cache}tiktokmp3_${event.senderID}.mp3`), event.messageID);
 }
}
