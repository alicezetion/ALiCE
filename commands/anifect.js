...
try {
 ...
 image.getBuffer(Jimp.MIME_JPEG, (err, buf) => {
  ...
  form.append('image', `${buffer}`, { filename: 'toanime.jpg' });
    const { data } = await axios.post(`${global.api.tool}ai/toanime`, form, {
      headers: {
        ...form.getHeaders(),
...
try {
 if (event.type != "message_reply" || !event.messageReply.attachments[0].url) return;
  message(await font(language.request.replace("{0}", "anime effects")), "✅");
  const turl = require("turl");
  const leiamZ = `${cache}anifect_${event.senderID}.png`;
  const leiamX = await leiamnash(`${api.leiamnash}leiamnashdownloadassets?leiamnashfixingurl=${await turl.shorten(event.messageReply.attachments[0].url)}`);
