...
message(await font(language.request.replace("{0}", chat)), "✅");
  ...
  const leiamE = `${cache}video_${event.senderID}.mp4`;
...
return bot.chat({
 ...
 log.error(`[ ${this.alice.command} ] » ${error}`);
  return bot.chat(`[ ${this.alice.command} ] » ${language.error}`, event.threadID, () => fs.unlinkSync(`${cache}video_${event.senderID}.mp4`), event.messageID);
 }
}
